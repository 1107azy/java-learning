...
public class Main {
    ...
    public static int getCount() {
        ...
        while (true) {
            try {
                ...
                if (n >= 1) {
                    scanner.nextLine(); // 吸收换行，避免影响后续 nextLine()
                    return n;
                } else {
    ...
    public static boolean isContinue(Scanner scanner) {
        while (true) {
            System.out.print("是否进行下一轮游戏（y/n）："); // 改用 print，更美观
    ...
    public static void main(String[] args) {
        ...
        if (!recordMap.isEmpty()) {
            ...
            // 3. 没有历史数据，走新建玩家流程
            int count = getCount(); // getCount 中已经处理了换行
        ...
        for (Player p : list) {
            ...
            game.start(p);
            p.showScore();
            int firstScore = ((GuessNumber) game).getSingleScore();
            recordMap.get(p).add(firstScore);
        }
